...
// Memantau perubahan class_id
watch(
    () => form.class_id,
    (newValue) => {
        console.log("Class ID selected:", newValue); // Debug class_id yang terpilih
...
const getSections = (classId) => {
    axios
        ...
        .then((response) => {
            sections.value = response.data; // Asumsikan response.data berisi array sections
        })
        .catch((error) => {
            console.error("Error fetching sections:", error); // Debug jika terjadi error
        });
};
